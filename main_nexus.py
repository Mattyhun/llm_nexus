...
from vm_communication import run_command_on_vm, transfer_file_to_vm
...
def main():
    ...
    try:
        samples = generate_answer_samples(model, problems, num_samples_per_task=1, max_threads=optimal_number_of_threads, requests_per_minute=3500)
    ...
    vm_output = run_command_on_vm(f"python3 human-eval/human_eval/evaluate_functional_correctness.py {samples_destination_path}")
